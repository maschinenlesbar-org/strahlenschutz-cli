...
import { StrahlNetworkError } from "../src/client/errors.js";
...
test("performs a real GET and returns status, headers and body", async () => {
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({ method: "GET", url: `${baseUrl}/ogc/opendata/ows` });
      assert.equal(resp.status, 200);
      assert.equal(resp.headers["content-type"], "application/json");
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), { path: "/ogc/opendata/ows" });
    },
  );
});

test("rejects an unsupported protocol with StrahlNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof StrahlNetworkError && /timed out after 300ms/.test(err.message),
      );
